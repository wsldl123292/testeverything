...
public class Shell {
    ...
    private Expect4j getExpect() {
        try {
            ...
            final localUserInfo ui = new localUserInfo();
            session.setUserInfo(ui);
            session.connect();
            channel = (ChannelShell) session.openChannel("shell");
            final Expect4j expect = new Expect4j(channel.getInputStream(), channel
                    .getOutputStream());
            channel.connect();
    ...
    public boolean executeCommands(String[] commands) {
        ...
        final List<Match> lstPattern = new ArrayList<>();
        final String[] regEx = linuxPromptRegEx;
        if (regEx != null && regEx.length > 0) {
            synchronized (regEx) {
                for (String regexElement : regEx) {// list of regx like, :>, />
                    ...
                    // remote machine
                    try {
                        final RegExpMatch mat = new RegExpMatch(regexElement, closure);
                        lstPattern.add(mat);
        ...
        try {
            ...
            //找不到错误信息标示成功
            final String response = buffer.toString().toLowerCase();
    ...
    private boolean isSuccess(List<Match> objPattern, String strCommandPattern) {
        try {
            final boolean isFailed = checkResult(expect.expect(objPattern));
            if (!isFailed) {
    ...
    public static void main(String[] args) {
        ...
        String cmd[] = {"sh /root/saibowisdom/stream/Saibo-Stop.sh"};
